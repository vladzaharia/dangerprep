...
# Usage: helpers/configuration.sh {audit|permissions|services|network|help}
...
if [[ "${SECURITY_HELPERS_CONFIGURATION_LOADED:-}" == "true" ]]; then
    return 0
fi
readonly SECURITY_HELPERS_CONFIGURATION_LOADED="true"
...
export -f cleanup_on_errornexport -f init_scriptnexport -f show_helpnexport -f check_permissionsnexport -f audit_servicesnexport -f check_network_securitynexport -f run_auditnexport -f generate_audit_reportn
