...
manage_secrets() {
    ...
    case "$operation" in
        "generate")
            ...
            echo
            # Check if generate-secrets.sh exists, if not use setup-secrets.sh
            if [[ -f "${SCRIPT_DIR}/generate-secrets.sh" ]]; then
                "${SCRIPT_DIR}/generate-secrets.sh"
            else
                log "Using setup-secrets.sh for secret generation"
                "${SCRIPT_DIR}/setup-secrets.sh" --force
            fi
            ;;
        "update")
...
manage_certificates() {
    ...
    case "$operation" in
        "status")
            ...
            echo
            "${SCRIPT_DIR}/certificate-manager.sh" status
            ;;
        "generate")
            show_banner_with_title "Certificate Generation" "security"
            echo
            "${SCRIPT_DIR}/certificate-manager.sh" generate
            ;;
        "renew")
            show_banner_with_title "Certificate Renewal" "security"
            echo
            "${SCRIPT_DIR}/certificate-manager.sh" renew
            ;;
        "validate")
            show_banner_with_title "Certificate Validation" "security"
            echo
            "${SCRIPT_DIR}/certificate-manager.sh" validate
            ;;
        *)
...
manage_monitoring() {
    ...
    case "$operation" in
        "suricata")
            ...
            echo
            "${SCRIPT_DIR}/suricata-monitor.sh"
            ;;
        "status")
