  ...
  return (
    <div className="wa-stack wa-gap-s">
      {connectedClients.map((client: ConnectedClient, index: number) => (
        <wa-details key={client.macAddress || index} summary={client.hostname || client.ipAddress || client.macAddress}>
          <div className="wa-stack wa-gap-xs">
            {client.ipAddress && (
              <div><strong>IP Address:</strong> {client.ipAddress}</div>
            )}
            <div><strong>MAC Address:</strong> {client.macAddress}</div>
            {client.hostname && (
              <div><strong>Hostname:</strong> {client.hostname}</div>
            )}
            {client.signalStrength && (
              <div><strong>Signal Strength:</strong> {client.signalStrength} dBm</div>
            )}
            {client.txRate && (
              <div><strong>TX Rate:</strong> {client.txRate}</div>
            )}
            {client.rxRate && (
              <div><strong>RX Rate:</strong> {client.rxRate}</div>
            )}
            {client.connectedTime && (
              <div><strong>Last Activity:</strong> {client.connectedTime}</div>
            )}
          </div>
        </wa-details>
      ))}
