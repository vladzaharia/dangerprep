      ...
      <wa-card appearance="filled">
        <div className="wa-stack wa-gap-m">
          ...
          <div className="wa-stack wa-gap-xs">
            <div><strong>Status:</strong> <wa-badge variant={tailscaleInterface.status === 'connected' ? 'success' : 'danger'}>{tailscaleInterface.status}</wa-badge></div>
            {tailscaleInterface.tailnetName && (
              <div><strong>Tailnet:</strong> {tailscaleInterface.tailnetName}</div>
            )}
            {tailscaleInterface.ipAddress && (
              <div><strong>IP Address:</strong> {tailscaleInterface.ipAddress}</div>
            )}
            {tailscaleInterface.exitNode !== undefined && (
              <div><strong>Exit Node:</strong> {tailscaleInterface.exitNode ? 'Yes' : 'No'}</div>
            )}
          </div>
        </div>
      </wa-card>
        ...
        {peers.length === 0 ? (
          ...
          peers.map((peer: TailscalePeer, index: number) => (
            <wa-details key={peer.ipAddress || index} summary={peer.hostname || peer.ipAddress}>
              <div className="wa-stack wa-gap-xs">
                <div><strong>IP Address:</strong> {peer.ipAddress}</div>
                <div><strong>Status:</strong> <wa-badge variant={peer.online ? 'success' : 'neutral'}>{peer.online ? 'Online' : 'Offline'}</wa-badge></div>
                {peer.os && (
                  <div><strong>OS:</strong> {peer.os}</div>
                )}
                {peer.lastSeen && (
                  <div><strong>Last Seen:</strong> {new Date(peer.lastSeen).toLocaleString()}</div>
                )}
                {peer.exitNode && (
                  <div><strong>Exit Node:</strong> Yes</div>
                )}
              </div>
            </wa-details>
          ))
